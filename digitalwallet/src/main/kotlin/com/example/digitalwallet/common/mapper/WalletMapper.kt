...
interface WalletMapper {
    ...
    @Mapping(source = "wallet.user.email", target = "user.email", qualifiedByName = ["emailValueObjectToString"])
    @Mapping(
        source = "wallet.user.document",
        target = "user.document",
        qualifiedByName = ["documentValueObjectToString"]
    )
    fun toModel(wallet: WalletDto): WalletModel
